    ...
    <div class="full-width flex flex-center" style="min-height: calc(100vh - 120px)">
      <q-card flat bordered class="bg-white" style="width: 460px; max-width: 92vw">
        <q-card-section class="q-pa-lg">
          <div class="text-h5 text-weight-bold text-grey-9 q-mb-sm">{{ t('login.title') }}</div>
          ...
          <div class="column q-gutter-sm">
            <q-btn
              ...
              no-caps
              :icon="githubIcon"
              :label="t('common.connectWithGithub')"
              color="grey-9"
              class="q-py-sm"
...
const connectWithGithub = async () => {
  if (!API) {
    $q.notify({ color: 'negative', message: t('login.backendNotConfigured') })
    return
  }

  try {
    const health = await fetch(`${API}/health`)
    if (!health.ok) throw new Error(t('login.backendUnavailable'))
    ...
    $q.notify({
      color: 'negative',
      message: error?.message || t('login.githubLoginFailed'),
    })
  }
}
...
const continueAsGuest = async () => {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new Error(payload?.message || t('login.guestLoginFailed'))
    }
    ...
    $q.notify({ color: 'negative', message: error?.message || t('login.guestLoginFailed') })
  }
}
</script>
